...
class BillingService:
    ...
    def generate_invoice(data, user=None):
        ...
        # 2. Find Student Enrollment - STRICT LOOKUP, NO AUTO-CREATION
        # Student must already be enrolled in the session matching the fee structure
        ...
        if not enrollment:
            # Clear error message indicating what enrollment is missing
            raise ValidationError(
                f"Student is not enrolled in {structure.grade.name} {structure.term.name} "
                f"{structure.academic_year.name}. Please complete enrollment before generating an invoice."
            )
