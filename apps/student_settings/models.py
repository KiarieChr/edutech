...
class Term(BaseModel):
    ...
    def save(self, *args, **kwargs):
        # Automate the active status based on start and end dates, 
        # but ALWAYS force status to 'active' if it is the manually set current term.
        today = timezone.now().date()
        if self.is_current:
            self.status = 'active'
        ...
        if self.is_current:
            # Ensure only one term is globally active at any time
