...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filter by tenant if available and not public
        if hasattr(self.request, 'tenant') and self.request.tenant.schema_name != 'public':
            qs = qs.filter(tenants=self.request.tenant)
    ...
    def perform_create(self, serializer):
        ...
        # Automatically assign user to the current tenant
        if hasattr(self.request, 'tenant') and self.request.tenant.schema_name != 'public':
    ...
    def dashboard_stats(self, request):
        ...
        stats = {
            ...
            'students': qs.filter(is_student=True).count(),
            'lecturers': qs.filter(is_lecturer=True).count(),
            'parents': qs.filter(is_parent=True).count(),
            'department_heads': qs.filter(is_dep_head=True).count(),
            'admins': qs.filter(is_superuser=True).count(),
            'active_users': qs.filter(is_active=True).count(),
            'inactive_users': qs.filter(is_active=False).count(),
            'first_login_pending': qs.filter(is_first_login=True).count(),
        }
    ...
    def filtered_list(self, request):
        ...
        # Start with all users filtered by tenant
