    ...
    def validate(self, data):
        ...
        if user:
            ...
            if request and hasattr(request, 'tenant') and request.tenant.schema_name != 'public':
                if not user.is_superuser and not user.tenants.filter(id=request.tenant.id).exists():
                    raise serializers.ValidationError(_("You do not have access to this school."))
        ...
        return [
            {
                "id": t.id,
                "name": t.name,
                "domain": t.domains.first().domain if t.domains.exists() else None
            } for t in obj.tenants.prefetch_related('domains').all()
        ]
