...
class SupplierInvoiceViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Convert QueryDict to a regular dict so DRF's ListSerializer
        # doesn't attempt HTML form parsing for nested 'lines' field
    ...
    def summary(self, request):
        ...
        total_payables = qs.filter(
            status__in=['POSTED', 'APPROVED'], balance__gt=0
        ).aggregate(total=Sum('balance'))['total'] or 0
        ...
        overdue_count = qs.filter(
            status__in=['POSTED', 'APPROVED'],
            due_date__lt=today, balance__gt=0
        ).count()

        voucher_qs = PaymentVoucher.objects.all()
        pending_vouchers = voucher_qs.filter(
            status__in=['DRAFT', 'PENDING_APPROVAL']
        ).count()
        paid_vouchers = voucher_qs.filter(status='PAID').count()

        refund_count = voucher_qs.filter(
            voucher_type='REFUND', status='PAID'
        ).count()

        pending_retirements = ImprestRetirement.objects.filter(
