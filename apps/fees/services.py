...
def ensure_session_enrollment(student_id, grade, term, year, curriculum=None):
    ...
    # Sync is_active with student_settings.Enrollment if possible
    settings_enr = Enrollment.objects.filter(
        student_id=student_id, grade=grade, term=term, academic_year=year
    ).first()
    
    # If the corresponding academic session is active, or if it's the current term, make it active.
    # Otherwise, don't disrupt the student's current active session.
    should_be_active = settings_enr.is_active if settings_enr else getattr(term, 'is_current', False)
    ...
    def generate_invoice(data, user=None):
        ...
        if existing_invoice:
            raise ValidationError(
                ...
                f"term. Void the existing invoice before creating a new one."
            )
